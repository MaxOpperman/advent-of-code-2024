...
def five_a(
    page_order=list[tuple[int, int]], updates=list[str]
) -> tuple[int, list[int]]:
...
def check_order(
    page_order=list[tuple[int, int]], updates=list[int]
) -> tuple[bool, int]:
    seen = []
...
def five_b(page_order=list[tuple[int, int]], updates=list[str]) -> int:
    ...
    for u in tqdm(updates):
        ...
        while not correct_order:
            update_pages[abs(ind - swap_i)], update_pages[ind] = (
                update_pages[ind],
                update_pages[abs(ind - swap_i)],
            )
            partial_correct_order, ind = check_order(
                current_page_rules, update_pages[:ind]
            )
